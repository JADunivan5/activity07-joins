...
| `zip`       |  int | zip code of laquinta location       |
| `longitude` |  num | east-west position on Earth         |
| `latitude`  |  num | north-south position on Earth       |
...
|----------------|------|-------------------------------|
| `name`         | chr  | state name                    |
| `abbreviation` | num  | state abbreviation            |
...
**Response**: There are no Denny's outside the US, but there is La Quinta's that are outside the US
...
**Response**: You could use if statements to exclude the locations outside of the US.
...
```{r filter}
filter(dennys, state$abbreviation=="NA")
```
...
```{r Anti_join, message=FALSE}
dennys %>% anti_join(states, by= ("state"="abbreviation")
```
